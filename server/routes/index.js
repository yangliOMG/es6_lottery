...
var makeIssue=function(){
	...
	if(date.getTime()-first_issue_date.getTime()>0&&date.getTime()-end_issue_date.getTime()<0){
		...
		cur_issue_date.setSeconds(0);	//9点
		var minus_time=date.getTime()-cur_issue_date.getTime();
		var h=Math.ceil(minus_time/1000/60/10);//xx期
		var end_date=new Date(cur_issue_date.getTime()+1000*60*10*h);//xx期的截止时间
...
// get opencode
router.get('/get/opencode', function (req, res, next) {//express格式
