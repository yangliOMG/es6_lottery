	...
	var Lottery = function (_mix) {
	  ...
	  _createClass(Lottery, [{
	    ...
	    value: function initEvent() {
	      var self = this;
	      (0, _jquery2.default)('#plays').on('click', 'li', self.changePlayNav.bind(self)); //bind(...)  ,可以把...传到函数this当中，
	      ...
	      (0, _jquery2.default)('.cz-syylogo').on('click', self.getOmit.bind(self));
	    }
	  }]);
	...
	var Base = function () {
		...
		_createClass(Base, [{
			...
			value: function initNumber() {
				for (var i = 1; i < 12; i++) {
					this.number.add(('' + i).padStart(2, '0')); //01  02 03添加头部0
				}
			}
		...
		_createClass(Timer, [{
			...
			value: function countdown(end, update, handle) {
				...
				if (now - end > 0) {
					...
					update.call(self, r.join('')); //第一个参数作为this传进去了，第二个参数作为变量传进去了，第三个类推
					setTimeout(function () {
